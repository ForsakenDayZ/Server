...
private ["_vehicle","_started","_finished","_animState","_isMedic","_vehicleNameText","_towTruckNameText","_towTruck","_isTowing","_hasToolbox","_findNearestBuildables","_findNearBuildable","_IsNearBuildables","_IsNearPlot"];
...
if(_IsNearBuildables > 0) exitWith {  DZE_ActionInProgress = false; cutText ["Cannot detach tow while nearby a player base.", "PLAIN DOWN"] };
...
if(_isTowing) then {
	...
	if(!(isNull _vehicle)) then {
		...
		if (_finished) then {
			...
			_vehicle setVariable ["Towed", false, true];
