...
private ["_vehicle","_started","_finished","_animState","_isMedic","_abort","_vehicleNameText","_towTruckNameText","_findNearestVehicles","_findNearestVehicle","_IsNearVehicle","_towTruck","_towableVehicles","_towableVehiclesTotal","_vehicleOffsetY","_towTruckOffsetY","_offsetZ","_hasToolbox","_findNearestBuildables","_findNearBuildable","_IsNearBuildables","_IsNearPlot"];

// check for near buildables
_findNearestBuildables = nearestObjects [player, ["ModularItems"], 30];
_findNearBuildable = [];

{
	if (alive _x) then {
		_findNearBuildable set [(count _findNearBuildable),_x];
	};
} count _findNearestBuildables;

_IsNearBuildables = count (_findNearBuildable);

// If item is is Near Buildables
if(_IsNearBuildables > 0) exitWith {  DZE_ActionInProgress = false; cutText ["Cannot attach tow while nearby a player base.", "PLAIN DOWN"] };
	...
	if (_finished) then {
		if(((vectorUp _vehicle) select 2) > 0.5) then {
			if( _towableVehiclesTotal > 0 ) then {
				...
				_vehicle setVariable ["Towed", true, true];
