	...
	if (count AllowedVehiclesList == 0) then {
		...
		if(_isShip || _isAir) then {
			if(_isShip) then {
				...
				waitUntil{!isNil "BIS_fnc_findSafePos"};
				_position = [MarkerPosition,0,DynamicVehicleArea,10,0,1,0] call BIS_fnc_findSafePos;
				//Normal _position = [MarkerPosition,0,DynamicVehicleArea,10,0,2000,0] call BIS_fnc_findSafePos;
