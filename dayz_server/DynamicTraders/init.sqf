...
_trader_config = [
	[8, "ATM.sqf", 		"Banker", 					"ColorBlue"],
	[4, "black_mkt.sqf", 		"Black Market", 					"ColorBlack"]/*,
	...
	for [{_i=0}, {_i<(_x select 0)}, {_i=_i+1}] do {
		...
		while {!_found_position} do {
			...
			if ((_x select 1) == "boat.sqf") then {
				_position = [getMarkerPos 'center',0,DynamicVehicleArea,20,0,1,1] call BIS_fnc_findSafePos;
			} else {
				if (!isNil "RoadList" and {(random 1) > 0.5}) then {
					...
					waitUntil{!isNil "BIS_fnc_findSafePos"};
					_position = [_position,5,40,20,0,1,0] call BIS_fnc_findSafePos;
				} else {
					waitUntil{!isNil "BIS_fnc_selectRandom"};
					_position = RoadList call BIS_fnc_selectRandom;
					_position = _position modelToWorld [0,0,0];
