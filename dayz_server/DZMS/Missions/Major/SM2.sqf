...
[nil,nil,rTitleText,"Sector B Radio Freq:\nIncoming air drop, have your men ready to secure the package.", "PLAIN",10] call RE;
...
while {_loop} do {
	if (!Alive _plane OR !Alive _pilot) then {
		...
		[nil,nil,rTitleText,"Sector B Radio Freq:\nMayday! Mayday! We have been hit!", "PLAIN",10] call RE;
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 1200) AND (!(_half))) then {
		[nil,nil,rTitleText,"Sector B Radio Freq:\nWe are 1200m out from the drop point!", "PLAIN",10] call RE;
	...
	if ((Alive _plane) AND (Alive _pilot) AND ((_plane distance _wp_pos) <= 200)) then {
		...
		[nil,nil,rTitleText,"Sector B Radio Freq:\nPackage dropped at the target site.", "PLAIN",10] call RE;
...
[nil,nil,rTitleText,"Sector B Radio Freq:\nControl, The supply drop has been raided!", "PLAIN",6] call RE;
